...
- **Final bootstrap components** (all in `api-server/src/lib`, routes in `routes/analysis.ts` + `routes/evolution.ts`): Evolution Scheduler (in-memory interval, disabled by default; env `EVOLUTION_SCHEDULER_ENABLED` / `EVOLUTION_SCHEDULER_INTERVAL_MINUTES`; `GET/POST /evolution/scheduler`), Quality/Debt/Dependency Analyzers + Architecture Validator (`GET /analysis/{quality,debt,dependencies,architecture}`), Refactoring Engine (`POST /analysis/refactor-plan` → improvements → convert → backlog-driven evolution; never edits code directly), Roadmap Generator (`GET /roadmap`), Internal Knowledge Base (`GET /knowledge-base/search?q=`), Documentation Generator (`POST /docs/generate` → `SELF_MODEL.md` at repo root, regenerated — do not hand-edit).
