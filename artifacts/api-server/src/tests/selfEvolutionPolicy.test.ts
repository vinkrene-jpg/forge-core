...
test("absolute stop actions and content are detected", () => {
  ...
  const costReasons = detectAbsoluteStopReasons(manifest, [
    { path: "index.js", content: 'fetch("https://api.openai.com/v1/chat/completions")' },
  ]);
  assert.ok(costReasons.some((reason) => reason.includes("kostactie")));
});
