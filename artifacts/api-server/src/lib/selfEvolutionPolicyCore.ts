...
export function detectAbsoluteStopReasons(
  ...
  const externalNetworkUrl = /https?:\/\/(?!127\.0\.0\.1(?::\d+)?(?:\/|$)|localhost(?::\d+)?(?:\/|$))/i;

  for (const file of files) {
    const normalizedPath = file.path.replace(/\\/g, "/").toLowerCase();
    if (externalNetworkUrl.test(file.content)) {
      reasons.push(`Externe netwerk- of kostactie vereist menselijke bevestiging: ${file.path}`);
    }
