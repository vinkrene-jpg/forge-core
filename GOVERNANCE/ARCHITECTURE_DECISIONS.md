...
Production WorkspaceExecutor verification uses a dedicated Docker image with dependencies and trusted command manifests baked in. Each run resolves its configured image reference to a local immutable SHA-256 image ID before container creation, mounts only a sanitized read-only snapshot of `sandbox/`, `lib/` and `artifacts/`, disables networking, drops capabilities, applies memory and process limits and permits writes only in disposable tmpfs mounts. Package installation, lifecycle scripts and dependency repair remain disabled during verification. Candidate TypeScript receives a forced build plus an explicit Forge Runtime typecheck; verification evidence persists bounded lengths, hashes, image ID and container ID but never raw command output. Container failure, timeout and cancellation remove the container and preserve WorkspaceExecutor rollback authority.
