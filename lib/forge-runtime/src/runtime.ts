...
export class ForgeRuntime {
  ...
  async #executeAutonomousCycle(
    ...
    if (input.objectiveProfile === "generic-build") {
      ...
      if (!Array.isArray(rawTargets) || rawTargets.length === 0) {
        const blocked = new Error(
          "generic-build requires explicit target files in mission.input.targets; execution blocked before provider invocation.",
        ) as MissionExecutionFailure;
        blocked.missionResultStatus = "blocked";
        blocked.missionResultCause = "missing-targets";
    ...
    const composition = await this.#operatorCore.composePrompt({
      ...
      taskType: input.objectiveProfile === "generic-build" ? "coding" : "analysis",
      privacy: "standard",
      budget: "medium",
      files: input.files,
      memoryKinds: [
